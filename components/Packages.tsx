    ...
    <section id="packages" className="pt-32 pb-20 px-4 bg-gray-50">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-end mb-10">
          ...
          <button className="text-sm font-semibold text-gray-500 hover:text-[#1a3c6e] transition-colors">View all packages</button>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {packagesData.map((pkg) => (
            <div key={pkg.id} className="bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col h-full group">
              ...
              {/* Input for Travellers (Mock) */}
              <div className="px-5 py-2 mt-auto">
                 <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg p-1">
                    <span className="text-xs font-bold text-gray-500 pl-2">Travelers:</span>
                    <div className="flex items-center">
                       <button className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-[#1a3c6e] font-bold transition-colors">-</button>
                       <span className="text-sm font-bold text-[#1a3c6e] w-4 text-center">1</span>
                       <button className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-[#1a3c6e] font-bold transition-colors">+</button>
                    </div>
              ...
              <div className="bg-[#dcfce7] p-4 flex items-center justify-between gap-3 mt-3">
                ...
                <div className="flex gap-2">
                   <button 
                      onClick={() => onBook(pkg)}
                      className="bg-[#1a3c6e] text-white text-xs font-bold px-4 py-2 rounded-lg shadow hover:bg-[#153059] transition-colors transform active:scale-95"
                   >
                      Book Now
                   </button>
                   <button className="bg-transparent border border-[#1a3c6e] text-[#1a3c6e] px-3 py-2 rounded-lg hover:bg-[#1a3c6e] hover:text-white transition-colors flex items-center gap-1">
                      <FileDown size={14} />
